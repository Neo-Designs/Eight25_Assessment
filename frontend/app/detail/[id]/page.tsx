...
import { 
  ...
  Heading, ShieldCheck, MessageSquare, Send, Loader2
} from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-slate-100">
        <div className="max-w-md w-full bg-slate-900 border border-rose-500/20 p-8 rounded-3xl text-center space-y-4 shadow-2xl">
          ...
          <button
            onClick={() => router.push('/')}
            className="w-full bg-slate-850 hover:bg-slate-850 text-white font-medium py-2 rounded-xl transition text-sm"
          >
            Return Home
          </button>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* General metrics */}
            <div className="bg-slate-900 border border-slate-855 rounded-2xl p-6 space-y-5">
            ...
            <div className="space-y-4">
              ...
              {auditData.audit_output.recommendations.map((rec, idx) => (
                <div key={idx} className="bg-slate-900 border border-slate-850 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="bg-slate-955 p-3 rounded-lg border border-slate-850 text-[11px] text-slate-400">
            ...
            <div className="bg-slate-900 border border-slate-850 rounded-3xl p-5 space-y-4">
              <div className="flex items-center space-x-2 text-indigo-400 border-b border-slate-850 pb-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">Ask questions about this audit's findings</p>
                </div>
              </div>
              
              <div className="max-h-80 overflow-y-auto space-y-3 p-3 bg-slate-950 rounded-2xl border border-slate-850/80">
                {messages.length === 0 ? (
                  <div className="py-10 text-center space-y-2">
                    <MessageSquare className="h-6 w-6 text-slate-700 mx-auto" />
                    <p className="text-slate-500 text-sm">Ask anything about this audit.</p>
                    <p className="text-slate-600 text-xs">e.g. "Why is my SEO score low?" or "What should I fix first?"</p>
                  </div>
                ) : (
                  messages.map((msg, idx) => (
                    <div key={idx} className={`rounded-xl ${
                      msg.role === 'user'
                        ? 'bg-indigo-950/50 border border-indigo-900/60 px-4 py-3'
                        : 'bg-slate-900 border border-slate-800 px-4 py-3'
                    }`}>
                      <p className={`text-[10px] font-semibold uppercase tracking-widest mb-2 ${
                        msg.role === 'user' ? 'text-indigo-400' : 'text-slate-500'
                      }`}>
                ...
                {chatLoading && (
                  <div className="flex items-center gap-2 px-4 py-3 bg-slate-900 border border-slate-800 rounded-xl text-slate-500 text-sm">
              ...
              <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-500 p-3 rounded-xl text-white transition flex-shrink-0 disabled:opacity-50"
                >
